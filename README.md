...
+ PureQuant交易系统各大模块
  ...
  <img src="../../../Pictures/purequant模块图.jpg" style="zoom: 67%;" />

+ PureQuant分布式管理交易系统

  <img src="../../../Pictures/系统架构设计.jpg" style="zoom: 67%;" />
...
```python
【交易提醒】下单结果：{'合约ID': 'TRX-USDT-SWAP', '方向': '卖出平多', '订单状态': '完全成交', '成交均价': '0.01784', '数量': '1', '成交金额': 17.84} 撤单结果：None
```

如未完全成交，执行撤单。返回下单结果与撤单结果。

```python
【交易提醒】下单结果：{'合约ID': 'TRX-USDT-SWAP', '方向': '买入开多', '订单状态': '等待成交'} 撤单结果：{'合约ID': 'TRX-USDT-SWAP', '方向': '买入开多', '订单状态': '撤单成功'}
```

针对未完全成交然后撤单的情况 ，可以使用持仓同步模块，自动检测当前持仓与策略应持仓位是否匹配，如不匹配则自动同步。
...
# k线是公共数据，传入空的字符串也可
exchange = OKEXFUTURES(config.access_key, config.secret_key, config.passphrase)
